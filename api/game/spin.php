<?php
// ============================================================================
// SUROPARA V7.11.0 - LEVIATHAN ALGORITHMIC GRID ENGINE (TENJO PROTOCOL)
// ----------------------------------------------------------------------------
...
// 5. Tenjo (Pity) System: Mathematically forces Bonus at 777 dry spins.
...
try {
    ...
    $lapsSinceBonusPre = (int)($machine['laps_since_bonus'] ?? 0);
    ...
    // --- PHASE 5: V7.11 MATHEMATICAL GJP ENGINE (TRIGGER-LOCKED) ---
    ...
    // Strict Mathematical Trigger Lock
    if ($currentJackpot >= $gjpMax) {
        $heatZone = 'MUST_HIT';
        $heatPct = 100.0;
        $isGrandJackpot = true; 
    ...
    // --- PHASE 6: V7.11 TENJO (PITY) & GRID GENERATION ---
    
    // V7.11 Tenjo Protocol: Force a Regular Bonus (7s) at 777 dry laps
    $isTenjoDrop = false;
    if ($lapsSinceBonusPre >= 777 && !$bonusMode && !$isFreeSpin && !$isGrandJackpot) {
        $isTenjoDrop = true;
    }
    ...
    // Apply the dynamically adjusted hit rate (Bypassed if Tenjo is triggered)
    $isHit = $isTenjoDrop ? true : ($entropy[4] <= $adjustedHitRate);
    ...
    if ($isGrandJackpot) {
        ...
        // V7.11 Tenjo Override overrides the weighted RNG
        if ($isTenjoDrop) {
            $winSym = 3; // Force 7s (Regular Bonus)
        } else {
            $symWeights = [];
            if ($bonusMode === 'HEAVEN') {

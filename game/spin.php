<?php
// ============================================================================
// SUROPARA V2 - LEVIATHAN SLOT ENGINE v5.1 (Omni-Tracker Edition)
// Fix: Soft-Failing Telemetry to prevent 400 SQL Crashes
// ============================================================================
...
try {
    ...
    if ($isFreeze || $isTenjoHit) {
        ...
        if ($bonusSpinsLeft > 0 && $bonusMode !== 'HEAVEN') {
            ...
            if ($isHit) {
                if ($isSoftPity) { $allowed = [[7,70], [6,30]]; } 
                elseif ($targetRtp <= 15.0) { $allowed = [[7,75], [6,20], [4,4], [5,1]]; } 
                ...
                } else { foreach($paylines[$chosenLine] as $pos) $result[$pos] = $winSym; }
            } else {
                do {
    ...
    if ($spinWin > ($betAmount * 5000) && !$isGrandJackpot) $spinWin = $betAmount * 5000;
    ...
    if ($spinWin > 0) {
        $pdo->prepare("UPDATE users SET balance = balance + ?, pnl_lifetime = pnl_lifetime - ? WHERE id = ?")->execute([$spinWin, $spinWin + $vaultSiphon, $userId]);
    }
    ...
    // --- PHASE 7: OMNI-TRACKER (Wrapped in try/catch to prevent 400 crashes) ---
    try {
        $today = date('Y-m-d');
        ...
        // Soft fail: Silently ignores missing tables so the slot spin doesn't crash!
    }
    ...
    echo json_encode([
        'status' => 'success', 'stops' => $result, 'winning_lines' => $winningLines, 'win_amount' => $spinWin, 'vaulted_amount' => $vaultSiphon,
        'new_balance' => (float)$finalBal, 'free_spins' => $newFreeSpins, 'bonus_mode' => $bonusMode, 'bonus_spins_left' => $bonusSpinsLeft,
        'laps_since_bonus' => $lapsSinceBonus, 'momentum_multiplier' => $momentumMult, 'session_token' => $currentToken,
        'is_teaser' => $isTeaser, 'is_reach_eye' => $isReachEye, 'is_jackpot' => $isGrandJackpot, 'is_freeze' => $isFreeze, 'in_zone' => $inZone, 'level_up' => $levelUpData
    ]);

} catch (Exception $e) {
    if (isset($pdo) && $pdo->inTransaction()) $pdo->rollBack();
    http_response_code(400); 
    echo json_encode(['error' => $e->getMessage()]); // Front end will now read this!
}
?>